...
{
	openvol::GeometryPacket geometry_packet;
	...
	const std::vector<std::uint8_t> bytes =
		openvol::serialize_geometry_packet(geometry_packet);
	if (bytes.empty() ||
...

	{
		...
		while ((result = av_read_frame(input, packet)) >= 0)
		{
			if (packet->stream_index == geometry_stream_index)
			{
				...
				openvol::GeometryPacket decoded;
...
bool pack_openvol(const PackOptions& options)
{
