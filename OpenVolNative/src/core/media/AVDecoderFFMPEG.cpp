...
AVDecoderFFMPEG::AVDecoderFFMPEG()
	: IAVDecoder(),
	  m_container(new openvol::FFmpegMp4VolumetricContainer()),
	  m_video_frames(32),
...
bool AVDecoderFFMPEG::decode()
{
	...
	{
		openvol::ContainerPacket packet =
...
{
	auto drain_one = [this](
		std::deque<openvol::ContainerPacket>& packets,
		bool ready)
	{
...
bool AVDecoderFFMPEG::decode_packet(
	const openvol::ContainerPacket& packet)
{
	av_packet_unref(&m_packet);
	if (av_new_packet(
...
{
	openvol::GeometryPacket packet;
	if (!openvol::parse_geometry_packet(
		m_packet.data,
...
bool AVDecoderFFMPEG::geometry_end_of_stream() const
{
	return m_geometry_frames.state() ==
			openvol::QueueState::EndOfStream &&
...
std::string AVDecoderFFMPEG::get_last_error() const
{
	...
	if (m_video_frames.state() == openvol::QueueState::Error)
		return m_video_frames.error();
	if (m_geometry_frames.state() == openvol::QueueState::Error)
...
// 
// --------------------------------------------------------------------------
openvol::FrameMatchResult AVDecoderFFMPEG::get_video_data(
	double presentation_time,
	double tolerance,
...
//	LOG("DecoderFFMPEG::get_video_data - start");
	...
	*outputV = NULL;
	if (!m_initialised)
	{
		...
		return openvol::FrameMatchResult::NotReady;
	}
	...
	if (frame_time > presentation_time + tolerance)
		return openvol::FrameMatchResult::Missing;
