...
{
	return m_streamed_meshes.state() == openvol::QueueState::Open &&
		!m_streamed_meshes.full();
}

std::string GeometryDecoderDraco::get_last_error() const
{
	if (m_streamed_meshes.state() == openvol::QueueState::Error)
		return m_streamed_meshes.error();
	if (m_decoded_meshes.state() == openvol::QueueState::Error)
...
// get_mesh_data
// --------------------------------------------------------------------------
openvol::FrameMatchResult GeometryDecoderDraco::get_mesh_data(
	double presentation_time,
	double tolerance,
