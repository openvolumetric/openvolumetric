#pragma once

#include <IAVDecoder.h>
#include <Mesh.h>
#include <TimedFrame.h>

#include <cstdint>
#include <memory>
#include <string>
#include <vector>

namespace openvol
{

/** Stable stream metadata returned without exposing FFmpeg structures. */
struct OpenVolMediaInfo
{
	int width = 0;
	int height = 0;
	double frame_rate = 0.0;
	double duration = 0.0;
	bool has_audio = false;
	int audio_sample_rate = 0;
	int audio_channels = 0;
};

/** One owned, timestamp-matched visual presentation. */
struct OpenVolPresentation
{
	double presentation_time = 0.0;
	int width = 0;
	int height = 0;
	std::vector<std::uint8_t> y;
	std::vector<std::uint8_t> u;
	std::vector<std::uint8_t> v;
	Mesh mesh;
};

/**
 * Engine-neutral façade for OpenVol playback.
 *
 * The façade owns FFmpeg and Draco workers and returns owned CPU data. Host
 * integrations never see codec types or retain decoder-owned frame pointers.
 */
class OpenVolPlayer final
{
public:
	OpenVolPlayer();
	~OpenVolPlayer();

	OpenVolPlayer(const OpenVolPlayer&) = delete;
	OpenVolPlayer& operator=(const OpenVolPlayer&) = delete;

	bool open(const char* path);
	bool start();
	void stop();
	void close();
	bool seek(double time);

	const OpenVolMediaInfo& media_info() const;
	const std::string& error() const;

	FrameMatchResult presentation(
		double requested_time,
		OpenVolPresentation& output);
	int read_audio(float* output, int sample_count);

private:
	bool submit_geometry(double requested_time);

	class Impl;
	std::unique_ptr<Impl> m_impl;
};

} // namespace openvol
