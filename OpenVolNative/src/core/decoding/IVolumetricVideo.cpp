...
bool IVolumetricVideo::prepare_presentation(double presentation_time)
{
	...
	while (std::chrono::steady_clock::now() < deadline)
	{
		...
		const auto video_result = m_avdecoder->get_video_data(
			...
			&u,
			&v);
		if (video_result == openvol::FrameMatchResult::Ready)
		{
			Mesh mesh;
			...
			if (geometry_result ==
				openvol::FrameMatchResult::Ready)
			{
				return true;
			}
			if (geometry_result ==
				openvol::FrameMatchResult::Missing)
			{
				LOG(
		...
		else if (video_result == openvol::FrameMatchResult::Missing)
		{
			LOG(
