...
OpenVolPlayer::OpenVolPlayer() : m_impl(std::make_unique<Impl>())
{
}

OpenVolPlayer::~OpenVolPlayer()
{
	close();
}

bool OpenVolPlayer::open(const char* path)
{
	close();
	if (path == nullptr || path[0] == '\0')
	{
		m_impl->error = "An OpenVol MP4 path is required.";
		return false;
	}
...
bool OpenVolPlayer::start()
{
	...
	{
		m_impl->error = "OpenVol decoder workers could not start.";
...
bool OpenVolPlayer::seek(double time)
{
...
const OpenVolMediaInfo& OpenVolPlayer::media_info() const
{
	return m_impl->info;
}

const std::string& OpenVolPlayer::error() const
{
	return m_impl->error;
}

bool OpenVolPlayer::submit_geometry(double requested_time)
{
...
FrameMatchResult OpenVolPlayer::presentation(
	double requested_time,
	OpenVolPresentation& output)
{
...
int OpenVolPlayer::read_audio(float* output, int sample_count)
{
	return m_impl->open
