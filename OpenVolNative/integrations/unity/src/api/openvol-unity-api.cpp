...
static void UNITY_INTERFACE_API OnRenderEvent(int eventID)
{
	...
	{
		LOG("openvol_quit - cant find instance id: %d", eventID);
		return;
	}
...
// --------------------------------------------------------------------------
// openvol_init - init plugin - returns instance id to access other functions
//
OPENVOL_API int openvol_init(int& ID)
{
	LOG("openvol_init - start");
	...
	{
		delete vv;
		LOG("openvol_init - unsupported or unavailable graphics device: %d", s_DeviceType);
		return -1;
	}
...
{
	LOG("openvol_quit - id: %d",ID);

	//
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_quit - cant find instance id: %d", ID);
		return;
	}
...
OPENVOL_API void openvol_set_time(int ID, double time)
{
	...
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_set_time - cant find instance id: %d", ID);
		return;
	}
...
{
	LOG("openvol_start_decoding - id: %d", ID);

	//
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_start_decoding - cant find instance id: %d", ID);
		return -1;
	}
...
{
	LOG("openvol_stop_decoding - id: %d", ID);

	//
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_stop_decoding - cant find instance id: %d", ID);
		return -1;
	}
...
{
	LOG("openvol_seek - id: %d - time: %f", ID, time);

	//
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_seek - cant find instance id: %d", ID);
		return -1;
	}
...
{
	LOG("openvol_load_video - id: %d", ID);

	// Get Instance
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_load_video - cant find instance id: %d", ID);
		return -1;
	}
	...
	{
		LOG("openvol_load_video - MP4 has no vvge geometry stream");
		return -1;
	}
	if (!(*iter)->get_geometrydecoder_ptr()->init())
	{
		LOG("openvol_load_video - failed to initialise embedded geometry");
		return -1;
	}
...
{
	LOG("openvol_get_video_details - id: %d", ID);

	// Get Instance
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_get_video_details - cant find instance id: %d", ID);
		return -1;
	}
...
OPENVOL_API int openvol_get_audio_details(
...
OPENVOL_API int openvol_read_audio(
...
{
	LOG("openvol_set_texture_pointer - id: %d", ID);

	// Get Instance
	VolumetricVideo_iter iter;
	if (!get_vv_instance(ID, &iter))
	{
		LOG("openvol_get_texture_pointers - cant find instance id: %d", ID);
		return -1;
	}
	...
	LOG("openvol_set_texture_pointer - %d x %d", width, height);
	...
	//
	LOG("openvol_set_texture_pointer - end");
	return 1;	
}
...
//
OPENVOL_API int	openvol_set_mesh_pointer(int ID, void* index_buffer_handle, int index_count, void* vertex_buffer_handle, int vertex_count)
{
	LOG("openvol_set_mesh_pointer - id: %d", ID);
