...
bool FOpenVolPlayerAdapter::Open(
	const FString& Path,
	FString& OutError)
{
...
bool FOpenVolPlayerAdapter::Start(FString& OutError)
{
	if (!Player.start())
	{
		OutError = UTF8_TO_TCHAR(Player.error().c_str());
		if (OutError.IsEmpty())
		{
			OutError = TEXT("OpenVol decoder workers could not start.");
		}
		return false;
	}
	OutError.Reset();
...
void FOpenVolPlayerAdapter::Stop()
{
	Player.stop();
}

bool FOpenVolPlayerAdapter::Seek(
	double TimeSeconds,
	FString& OutError)
{
	if (!Player.seek(TimeSeconds))
	{
		...
		if (OutError.IsEmpty())
		{
			OutError = TEXT("OpenVol seek failed.");
		}
		return false;
	}
	OutError.Reset();
...
void FOpenVolPlayerAdapter::Close()
{
	Player.close();
}

const openvol::OpenVolMediaInfo&
FOpenVolPlayerAdapter::GetMediaInfo() const
{
	return Player.media_info();
}

openvol::FrameMatchResult FOpenVolPlayerAdapter::PollPresentation(
	double TimeSeconds,
...
{
	openvol::OpenVolPresentation Presentation;
	...
	UVElements.Reserve(
		static_cast<int32>(Presentation.mesh.verts.size()));
	for (const openvol::Vertex& Vertex : Presentation.mesh.verts)
	{
		// OpenVol coordinates are metres with Y up. Unreal uses centimetres
		...
		// OpenVol's source convention has its texture origin at the opposite
...
int32 FOpenVolPlayerAdapter::ReadAudio(
	int32 SampleCount,
	TArray<int16>& OutSamples)
{
	const openvol::OpenVolMediaInfo& Info = Player.media_info();
	if (!Info.has_audio ||
