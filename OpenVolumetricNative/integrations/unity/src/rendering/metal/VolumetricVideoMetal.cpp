...
int VolumetricVideoMetal::render()
{
	...
	if (m_avdecoder->get_video_data(
		...
		&output_y,
		&output_u,
		&output_v) != openvolumetric::FrameMatchResult::Ready)
		return -1;
	...
	if (geometry_result == openvolumetric::FrameMatchResult::Missing)
	{
		LOG("SYNC dropping unmatched video pts=%f", video_time);
		m_avdecoder->clean_frame_data();
		return -1;
	}
	if (geometry_result != openvolumetric::FrameMatchResult::Ready)
		return -1;
